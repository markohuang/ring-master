...
params = Namespace(**dict(
    ...
    max_cand_size=20,
    cands_hidden_size=512,
))

def process_vocab(smiles):
    vocab = set()
    try:
        smiles = smiles.strip()
...
def setup_vocab(params):
    ...
    with open(params.smiles_path, 'r') as f:
        smiles_list = f.readlines()[:100]
    with mp.Pool(mp.cpu_count()) as pool:
        results = list(tqdm(pool.imap(process_vocab, smiles_list)))
        all_vocab = set().union(r for r in results if r is not None)
