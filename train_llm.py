...
from ringmaster.vae_model import ScaffoldVAE
vocab, tokenizer, trainset, valset = setup_experiment(cfg)
...
def my_collator(batch, tokenizer, grammar, max_atom_neighbor, max_motif_neighbor, key):
    graph_list = []
    smiles_list = [ b[key] for b in batch ]
    
    group_selfies = ['] '.join(grammar.full_encoder(Chem.MolFromSmiles(mol)).split(']')) for mol in smiles_list]
    selfies = tokenizer(
        group_selfies,
        truncation=True,
    ...
    return graph_data, slices, selfies['input_ids'], selfies['attention_mask']
...
if __name__ == "__main__":
    ...
    if hasattr(cfg['setupparams'], 'load_model'):
        ...
        model = ScaffoldVAE.load_from_checkpoint(model_path)
    else:
        model = ScaffoldVAE(cfg, vocab)
